...
timeVmemFileStr = 'timeVmemRM1c_' + IBiasStr + '_' + IZapMaxStr + '.txt'
file9 = open(timeVmemFileStr,'w')
for index in range(len(M.t)):
    file9.write(str(M.t[index] / ms) + " " + str(M[0].v[index] / mV) + '\n')
file9.close()

Zfft_CoreFileStr = 'ZfftRM1c_' + IBiasStr + '_' + IZapMaxStr
absZfft_CoreFileStr = 'absZfftRM1c_' + IBiasStr + '_' + IZapMaxStr
...
fig_absZ_RM1c, ax = plt.subplots(1, 1)
ax.set_title('Slow (RM Type 1c: gKLT = 0, gH/40)', fontsize=17)
ax.set_ylabel('impedance (MOhm)', fontsize=17)
ax.set_xlabel('frequency (Hz)', fontsize=17)
ax.set_ylim((0,900))
ax.tick_params(axis='both', which='major', labelsize=17)
ax.semilogx(range(10,2000),abs(Zfft[10:2000])/1e6, linewidth=2.0)
fig_absZ_RM1c.tight_layout()
fig_absZ_RM1c.savefig(absZfft_CoreFileStr + '.png', dpi=300)
