...
timeVmemFileStr = 'timeVmemRM2f_' + IBiasStr + '_' + IZapMaxStr + '.txt'
file0 = open(timeVmemFileStr,'w')
for index in range(len(M.t)):
    file0.write(str(M.t[index] / ms) + " " + str(M[0].v[index] / mV) + '\n')
file0.close()

Zfft_CoreFileStr = 'ZfftRM2f_' + IBiasStr + '_' + IZapMaxStr
absZfft_CoreFileStr = 'absZfftRM2f_' + IBiasStr + '_' + IZapMaxStr
file1 = open(Zfft_CoreFileStr  + '.txt','w')
file2 = open(absZfft_CoreFileStr  + '.txt','w')
for index in range(len(Zfft)):
    file1.write(str((index/len(Zfft)) * (1/defaultclock.dt)/Hz) + " " + str(Zfft[index]) + '\n')
    file2.write(str((index/len(Zfft)) * (1/defaultclock.dt)/Hz) + " " + str(abs(Zfft[index])) + '\n')
file1.close()
file2.close()

fig_absZ_RM2f, ax = plt.subplots(1, 1)
ax.set_title('Very Fast (2x gKLT, gH, gNa, gKHT)', fontsize=18)
ax.set_ylabel('impedance (MOhm)', fontsize=18)
ax.set_xlabel('frequency (Hz)', fontsize=18)
ax.set_ylim((0,30))
ax.tick_params(axis='both', which='major', labelsize=18)
ax.semilogx(range(10,2000),abs(Zfft[10:2000])/1e6,  linewidth=2.0)
fig_absZ_RM2f.tight_layout()
fig_absZ_RM2f.savefig(absZfft_CoreFileStr + '.png', dpi=300)
