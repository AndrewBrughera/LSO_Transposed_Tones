...
#plt.plot(range(5,900),abs(Vfft[5:900])) # For now, 1-Hz resolution, drop 0 Hz (large DC)
plt.semilogx(range(10,2000),abs(Vfft[10:2000])) # For now, 1-Hz resolution, drop 0 Hz (large DC)
plt.show()

#plt.plot(range(5,900),abs(Ifft[5:900])) # For now, 1-Hz resolution, drop 0 Hz (large DC)
plt.semilogx(range(10,2000),abs(Ifft[10:2000])) # For now, 1-Hz resolution, drop 0 Hz (large DC)
plt.show()

#plt.plot(range(5,900),abs(Zfft[5:900])) # For now, 1-Hz resolution, drop 0 Hz (large DC)
plt.semilogx(range(10,2000),abs(Zfft[10:2000])) # For now, 1-Hz resolution, drop 0 Hz (large DC)
plt.ylim((0,100e6))
plt.show()


timeVmemFileStr = 'timeVmemRM2z_' + IBiasStr + '_' + IZapMaxStr + '.txt'
file6 = open(timeVmemFileStr,'w')
for index in range(len(M.t)):
    file6.write(str(M.t[index] / ms) + " " + str(M[0].v[index] / mV) + '\n')
file6.close()

Zfft_CoreFileStr = 'ZfftRM2z_' + IBiasStr + '_' + IZapMaxStr
absZfft_CoreFileStr = 'absZfftRM2z_' + IBiasStr + '_' + IZapMaxStr
file7 = open(Zfft_CoreFileStr  + '.txt','w')
file8 = open(absZfft_CoreFileStr  + '.txt','w')
for index in range(len(Zfft)):
    file7.write(str((index/len(Zfft)) * (1/defaultclock.dt)/Hz) + " " + str(Zfft[index]) + '\n')
    file8.write(str((index/len(Zfft)) * (1/defaultclock.dt)/Hz) + " " + str(abs(Zfft[index])) + '\n')
file7.close()
file8.close()

fig_absZ_RM2z, ax = plt.subplots(1, 1)
ax.set_title('Moderate (0.5x gKLT, gH)', fontsize=18)
ax.set_ylabel('impedance (MOhm)', fontsize=18)
ax.set_xlabel('frequency (Hz)', fontsize=18)
ax.set_ylim((0,95))
ax.tick_params(axis='both', which='major', labelsize=18)
ax.semilogx(range(10,2000),abs(Zfft[10:2000])/1e6, linewidth=2.0)
fig_absZ_RM2z.tight_layout()
fig_absZ_RM2z.savefig(absZfft_CoreFileStr + '.png', dpi=300)
