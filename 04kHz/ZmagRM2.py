...
timeVmemFileStr = 'timeVmemRM2_' + IBiasStr + '_' + IZapMaxStr + '.txt'
file3 = open(timeVmemFileStr,'w')
for index in range(len(M.t)):
    file3.write(str(M.t[index] / ms) + " " + str(M[0].v[index] / mV) + '\n')
file3.close()

Zfft_CoreFileStr = 'ZfftRM2_' + IBiasStr + '_' + IZapMaxStr
absZfft_CoreFileStr = 'absZfftRM2_' + IBiasStr + '_' + IZapMaxStr
file4 = open(Zfft_CoreFileStr  + '.txt','w')
file05 = open(absZfft_CoreFileStr  + '.txt','w')
for index in range(len(Zfft)):
    file4.write(str((index/len(Zfft)) * (1/defaultclock.dt)/Hz) + " " + str(Zfft[index]) + '\n')
    file05.write(str((index/len(Zfft)) * (1/defaultclock.dt)/Hz) + " " + str(abs(Zfft[index])) + '\n')
file4.close()
file05.close()

fig_absZ_RM2, ax = plt.subplots(1, 1)
ax.set_title('Fast (RM Type 2: 1x)', fontsize=18)
ax.set_ylabel('impedance (MOhm)', fontsize=18)
ax.set_xlabel('frequency (Hz)', fontsize=18)
ax.set_ylim((0,55))
ax.tick_params(axis='both', which='major', labelsize=18)
ax.semilogx(range(10,2000),abs(Zfft[10:2000])/1e6,  linewidth=2.0)
fig_absZ_RM2.tight_layout()
fig_absZ_RM2.savefig(absZfft_CoreFileStr + '.png', dpi=300)
