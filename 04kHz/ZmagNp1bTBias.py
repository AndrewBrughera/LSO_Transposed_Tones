...
dv/dt = (ileak + ina + ikht + iklt + ika + ih + ihcno + I)/C : volt
vu = v/mV : 1  # unitless v
I = I_Bias + I_Zap_Max * sin(2*pi*(m2000*(t - (t_settle+t_bias)) + f_min) * (t - (t_settle+t_bias))) : amp
"""
#eqs = """
#dv/dt = (ileak + ina + ikht + iklt + ika + ih + ihcno + I)/C : volt
#vu = v/mV : 1  # unitless v
#if (t_settle <= t < (t_settle + sweepdur)):
#    I = I_Bias + I_Zap_Max * sin(2*pi*(m2000*(t - t_settle) + f_min) * (t - t_settle)): amp
#else:
#    I = 0: amp
#
#"""
...
t_settle = 50*ms
t_bias = 160*ms
#zap1000_0p96 = I_Zap_Max * sin(2*pi*(m2000*(t - t_settle) + f_min) * (t - t_settle));
...
IBiasStr = 'IBias' + str(int(I_Bias/pA)) + 'pA' + str(int(t_bias/ms)) + 'ms'
...
#plt.semilogx(range(10,2000),abs(Zfft[10:2000])/1e6) # For now, 1-Hz resolution, drop 0 Hz (large DC)
#plt.ylim((0,200e6))
#plt.show()

timeVmemFileStr = 'timeVmemNp1b_' + IBiasStr + '_' + IZapMaxStr + '.txt'
file9 = open(timeVmemFileStr,'w')
for index in range(len(M.t)):
    file9.write(str(M.t[index] / ms) + " " + str(M[0].v[index] / mV) + '\n')
file9.close()

Zfft_CoreFileStr = 'ZfftNp1b_' + IBiasStr + '_' + IZapMaxStr
absZfft_CoreFileStr = 'absZfftNp1b_' + IBiasStr + '_' + IZapMaxStr
...
fig_absZ_Np1b, ax = plt.subplots(1, 1)
#ax.set_title('model LSO neuron Np1b (900-ms pre-bias)', fontsize=16)
ax.set_title('model LSO neuron Np1b ' + IBiasStr, fontsize=16)
ax.set_ylabel('impedance (MOhm)', fontsize=16)
ax.set_xlabel('frequency (Hz)', fontsize=16)
ax.set_ylim((0,80))
ax.tick_params(axis='both', which='major', labelsize=16)
ax.semilogx(range(10,2000),abs(Zfft[10:2000])/1e6, linewidth=2.0)
fig_absZ_Np1b.tight_layout()
fig_absZ_Np1b.savefig(absZfft_CoreFileStr + '.png', dpi=300)

#fig_out, ax = plt.subplots(1, 1)
#ax.semilogx(range(10,2000),abs(Zfft[10:2000])/1e6,  linewidth=2.0, alpha=0.3, color='gray')
#inch = 2.54
