# -*- coding: utf-8 -*-
"""
do25LsoRM1aIPD6kHz.py

Calls a model LSO neuron type 1a (non-principal, moderately slow membrane, 
lson from lson25RM1a.py) with input from spikeTime files of model auditory 
nerve fibers (Zilany et al., 2014) for 25 stimulus repetitions of transposed 
tones at 6 kHz, with modulation rates given in variable modFreqs_Hz.
Input files are drawn from, and output files are written to, subfolders:
    stimReps\rep## where ## ranges from 01 to 25.

Created on Fri 30 Mar 2020

@author: Andrew Brughera
"""
import os
from lson25RM1a import lson
...
#os.chdir('C:\\Users\\Andrew Brughera\\lso\\IPD\\6kHz')
# will allow for an error message from gbCoSpkFiles.append('ANSpTsCaCF06kHz...
# if we don't start in the 6kHz directory.
os.chdir('stimReps')
 
#for stimRep in range(NStimReps):
for stimRep in range(23,NStimReps):
    ...
    for modF in modFreqs_Hz:
        ...
        for EPhsIdx in range(int(540/EPhsStep_deg)):
            gbCoSpkFiles.append('ANSpTsCaCF06kHz1TT'+ str(modF).zfill(3) + 'HzEPhsN'+ str(EPhsIdx*EPhsStep_deg).zfill(3) + 'SPL75dB' + stimRepPlus1Str + 'MdSp3.txt')
            sbIpSpkFiles.append('ANSpTsCaCF06kHz2TT'+ str(modF).zfill(3) + 'HzEPhsN'+ str(EPhsIdx*EPhsStep_deg).zfill(3) + 'SPL75dB' + stimRepPlus1Str + 'MdSp3.txt')
        ...
        # end of ITD shift loop
    # end of modF loop
    #os.chdir('C:\\Users\\Andrew Brughera\\lso\\IPD\\6kHz\\stimReps')
    os.chdir('..')
# end of stimRep loop
os.chdir('..')
os.getcwd() # should return 'C:\\Users\\Andrew Brughera\\lso\\IPD\\6kHz'
