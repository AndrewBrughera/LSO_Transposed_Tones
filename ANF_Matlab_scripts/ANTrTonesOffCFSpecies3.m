% ANTransposedTones.m
% Andrew Brughera
% Modified from ANTransposedTonesSpecies3.m (2019 May 20)
% which lists ANSR100kHzAMwFSPhsSpecies3.m as a source.
% Applying the Zilany et al. 2014 model 
% with transposed tones that are higher than CF
% as in Monaghan et al. (2015):
%
...
%
% Carrier frequencies will remain at 4,6 & 10 KHz
% CFs will be incremented by 0.28, 0.43, and 0.57 kHz
% to produce maximum AM at the output of peripheral auditory filters:
% CFs @ 4.28, 4.43 & 10.57 kHz as shown in Monaghan et al. (2015)
% and also @ 6.28, 6.43, 10.28 & 10.43 kHz
% to stimulate at their peaks in AM above 6 and 10 kHz.
...
CarrierFreqs_Hz = [4000 4000 6000 6000 10000 10000 10000];
% minimumZilanyCF_Hz = 56;
% CFs_Hz = max(CarrierFreqs_Hz,minimumZilanyCF_Hz);
CFs_Hz = [4280 4430 6280 6430 10280 10430 10570];
...
nANFsPerFiberType = 40; % 8 contraI (allow for 40), 40 ipsiE direct inputs to model LSO neuron,
% as in Gjion et al. 2018 (20 and 20 in Wang and Colburn, 2012).
        ...
        for idxModF = 1:length(ModFreqs_Hz)
            ...
            for idxEnvStartPhase = 1:length(EnvStartPhases_deg)
                ...
                for fiberType = 1:nFiberTypes
                    ...
                    filename = ['ANSpTs' 'CF' num2str(CF_Hz/1000) 'kHz' 'Ca' num2str(F0_Hz/1000) 'kHz' num2str(ear) 'EPhs' EnvStPhsDegString 'TT' num2str(ModFreqHz) 'Hz' num2str(stimI_dBSPLrms) 'dBspl' fiberTypeStringsShort{fiberType} 'Sp' num2str(species)];
